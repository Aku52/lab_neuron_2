...
#!!!!!!!!!! ДАННАЯ МОДЕЛЬ -- ИСПРАВЛЕННАЯ ВЕРСИЯ КОДА ФАЙЛА lz_2_16.py
...
def tabel_of_values(students_encoded,data):
...
def checking_1(knn,X_test,y_test):
...
def main():
    data = data_write()
    students_encoded = encoder_fiting(data)
    X, y = tabel_of_values(students_encoded, data)
    X_train, X_test, y_train, y_test = the_test_and_train(X, y)
    knn = knn_train(X_train, y_train)
    checking_1(knn, X_test, y_test)
    tree = decision_tree(X_train, y_train)
    hecking_2(tree, y_test, X_test)
    
    # Передаём encoder как DataFrame с нужным столбцом
    encoder = OneHotEncoder()
    encoder.fit(data[["students"]])
    example(encoder, knn)

    X_vis, y_vis = des_boundary(data)
    knn_vis = train_the_model(X_vis, y_vis)
    graph(knn_vis, X_vis, y_vis, data)
